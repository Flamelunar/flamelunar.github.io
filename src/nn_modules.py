...
class WeightNetwork(nn.ModuleList):
    def __init__(self, source_model, pairs):
        ...
        for i,_ in pairs:
            ...
            nn.init.orthogonal_(self[-1].weight)
...
class InputLayer(nn.Module):
    ...
    def forward(self, words, ext_words, tags, domains, domain_id, word_lens_encoder, char_idxs_encoder, subword_idxs,
        ...
        return x_final, domain_id #[1,9,200]
...
class MyLSTM(nn.Module):
    ...
    def _forward_rnn(cell, x, masks, initial, h_zero, in_drop_masks, hid_drop_masks_for_next_timestamp, is_backward):
        ...
        for t in range(max_time): #9
            if is_backward:
                t = max_time - t - 1
            input_i = x[t] #对每一个input进行门口然后droput再加入到output
            if in_drop_masks is not None:
                input_i = input_i * in_drop_masks
            h_next, c_next = cell(input=input_i, hx=hx)
            # padding mask
            h_next = h_next * masks[t]  # + h_zero[0]*(1-masks[t])  # element-wise multiply; broadcast 第一个词和第一个1相乘不变==不dropout
        ...
        if is_backward:
            output.reverse()
        output = torch.stack(output, 0) #最后把所有词的表示加起来变成一句话
        return output  # , hx
